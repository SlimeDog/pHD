...
public class Settings {
    ...
    public static enum SettingIssue {
        ...
        SHOW_TIME("defaults.show-time", SettingType.INTEGER_PLUS), SAVE_DELAY("save-frequency", SettingType.LONG_PLUS),
        ENABLE_METRICS("enable-metrics", SettingType.BOOLEAN),
        CHECK_FOR_UPDATES("check-for-updates", SettingType.BOOLEAN), STORAGE("storage-type", SettingType.STORAGE);
        ...
        public boolean fits(String val) {
            if (val == null)
                return false; // fallback to default is elsewhere
            switch (type) {
                case DOUBLE:
                case DOUBLE_PLUS:
                    try {
    ...
    public static enum SettingType {
        DOUBLE("DOUBLE"), DOUBLE_PLUS("DOUBLE > 0.0"), INTEGER("INT"), INTEGER_PLUS("INT > 0"), LONG("LONG"),
        LONG_PLUS("LONG > 0"), BOOLEAN("BOOL"), STORAGE("STORAGE");
